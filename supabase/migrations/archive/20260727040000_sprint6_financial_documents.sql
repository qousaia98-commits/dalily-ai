-- Sprint 6 Phase 4 — Invoices, Receipts & Financial Documents

-- Global company billing / invoice issuer settings
CREATE TABLE IF NOT EXISTS public.company_billing_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_name TEXT NOT NULL DEFAULT 'Dalily',
  company_address TEXT NOT NULL DEFAULT '',
  country TEXT NOT NULL DEFAULT 'Syria',
  vat_number TEXT NOT NULL DEFAULT '',
  tax_id TEXT NOT NULL DEFAULT '',
  support_email TEXT NOT NULL DEFAULT 'support@dalily.app',
  website TEXT NOT NULL DEFAULT 'https://dalily.app',
  phone TEXT NOT NULL DEFAULT '',
  invoice_footer TEXT NOT NULL DEFAULT 'Thank you for doing business with Dalily.',
...
INSERT INTO public.company_billing_settings (id)
SELECT gen_random_uuid()
WHERE NOT EXISTS (
  SELECT 1 FROM public.company_billing_settings WHERE is_active = true
);

-- Sequential number sequences (never reuse)
...
CREATE TABLE IF NOT EXISTS public.financial_documents (
  ...
  status TEXT NOT NULL DEFAULT 'issued'
    CHECK (status IN ('draft', 'issued', 'void', 'regenerated')),
  currency TEXT NOT NULL DEFAULT 'USD',
  ...
  regenerated_from UUID REFERENCES public.financial_documents(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS financial_documents_provider_idx
  ON public.financial_documents (provider_id, created_at DESC);
CREATE INDEX IF NOT EXISTS financial_documents_payment_idx
  ON public.financial_documents (payment_id);
CREATE INDEX IF NOT EXISTS financial_documents_type_idx
  ON public.financial_documents (document_type, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS financial_documents_payment_type_active_uidx
  ON public.financial_documents (payment_id, document_type)
  WHERE status IN ('issued', 'regenerated');
...
CREATE TABLE IF NOT EXISTS public.pdf_generation_logs (
  ...
  action TEXT NOT NULL
    CHECK (action IN ('generate', 'regenerate', 'fail')),
...
CREATE INDEX IF NOT EXISTS pdf_generation_logs_doc_idx
  ON public.pdf_generation_logs (document_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS document_download_history_doc_idx
  ON public.document_download_history (document_id, created_at DESC);
...
DROP POLICY IF EXISTS company_billing_admin ON public.company_billing_settings;
CREATE POLICY company_billing_admin ON public.company_billing_settings
  FOR ALL TO authenticated
  USING (public.has_role('admin'))
  WITH CHECK (public.has_role('admin'));

DROP POLICY IF EXISTS company_billing_read ON public.company_billing_settings;
CREATE POLICY company_billing_read ON public.company_billing_settings
  FOR SELECT TO authenticated
  USING (is_active = true);

DROP POLICY IF EXISTS financial_documents_own ON public.financial_documents;
CREATE POLICY financial_documents_own ON public.financial_documents
  FOR SELECT TO authenticated
  USING (
...
DROP POLICY IF EXISTS invoice_metadata_own ON public.invoice_metadata;
CREATE POLICY invoice_metadata_own ON public.invoice_metadata
  FOR SELECT TO authenticated
  USING (
    public.has_role('admin')
    OR EXISTS (
...
DROP POLICY IF EXISTS receipt_metadata_own ON public.receipt_metadata;
CREATE POLICY receipt_metadata_own ON public.receipt_metadata
  FOR SELECT TO authenticated
  USING (
    public.has_role('admin')
    OR EXISTS (
...
DROP POLICY IF EXISTS pdf_logs_admin ON public.pdf_generation_logs;
CREATE POLICY pdf_logs_admin ON public.pdf_generation_logs
  FOR SELECT TO authenticated
  USING (public.has_role('admin'));

DROP POLICY IF EXISTS download_history_admin ON public.document_download_history;
CREATE POLICY download_history_admin ON public.document_download_history
  FOR SELECT TO authenticated
  USING (public.has_role('admin'));

DROP POLICY IF EXISTS sequences_admin ON public.document_number_sequences;
CREATE POLICY sequences_admin ON public.document_number_sequences
  FOR SELECT TO authenticated
  USING (public.has_role('admin'));
...
COMMENT ON TABLE public.financial_documents IS
  'Sprint 6 Phase 4 — immutable financial documents (invoices/receipts).';
COMMENT ON FUNCTION public.next_document_number(TEXT) IS
  'Atomic sequential INV/RCP/CRN document numbers per year.';
